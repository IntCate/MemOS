...
export const useChatStore = defineStore('chat', {
  ...
  actions: {
    ...
    // 准备消息发送参数
    prepareMessageParams(content, model, reasoning, webSearchEnabled, modelParams) {
    ...
    // 发送消息（使用API服务）
    async sendMessage(content, model, reasoning = false, webSearchEnabled = false) {
      // 验证消息参数
      ...
      try {
        ...
        const { formattedModel, ragConfigToUse, messageParams } = this.prepareMessageParams(
          content, model, reasoning, webSearchEnabled, modelParams
        );
