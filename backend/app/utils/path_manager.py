...
class PathManager:
    ...
    def get_prompt_template_dir():
        """获取提示词模板目录"""
        return os.path.join(PathManager.get_user_data_dir(), 'prompts')
    
    @staticmethod
    def get_prompt_version_dir():
        """获取提示词版本目录"""
        return os.path.join(PathManager.get_user_data_dir(), 'prompt_versions')
    
    @staticmethod
    def get_memory_root():
        """获取记忆数据根目录"""
        return os.path.join(PathManager.get_user_data_dir(), 'memory')
